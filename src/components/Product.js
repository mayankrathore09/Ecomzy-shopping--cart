...
const Product = ({post}) => {
  ...
  return (
    <div className='flex flex-col items-center justify-between hover:scale-110 transition duration-300 ease-in
      shadow-[0_3px_10px_rgb(0,0,0,0.2)]  hover:shadow-[rgba(0,_0,_0,_0.4)_0px_30px_90px] gap-3 p-4 mt-10 ml-5 rounded-xl'>
        <div  >
          <p className='text-grey-700 font-semibold text-lg trucate w-40 mt-1 '>{post.title}</p>
        </div>
        <div>
          <p className="w-40 text-gray-400 font-normal text-[10px] text-left">{post.description.split(" ").slice(0,10).join(" ") + "..."}</p>
        </div>
        <div className="h-[180px]">
          <img className="h-full w-full " src={`${post.image}`} alt='ig'></img>
        </div>
